if(getget("id")){
	ajax("GET",AJAXURL+"getproduct/"+getget("id"),function(data,event){
		if(data["success"]){
			...
			formsubmit("form",function(){
				ajax("PUT",AJAXURL+"editproduct/"+getget("id"),function(data,event){
					if(data["success"]){
						alert("edit success")
						location.href="companydetail.html?companyid="+data["data"]
					}else{
