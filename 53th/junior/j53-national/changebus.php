<?php
    ...
    if(isset($_GET["change"])){
        $id = $_GET["change"];
        $car = $pdo->query("SELECT * FROM `bus` WHERE `id`=$id")->fetch();
        if(!$id){
            ?><script>alert("無此倆公車");location="./user.php"</script><?php
        }
    }else{
...
<body>
    ...
    <div class="border p-3 my-3" style="width: 1200px;position: relative;left: 20%;top: 40px;">
        <div class="d-flex" style="position: relative;width: 500px;left: 30%;">
            <h1 class="mx-3">修改「<?=$car["car"]?>」接駁車</h1>
        </div>
    </div>
    <form action="change1.php" method="post" id="form">
        <div style="width: 1200px;position: relative;top: 100px;left: 20%;">
            <div class="d-flex my-3">
                <label for="time" style="font-size: 30px;" class="w-25">已行駛時間(分鐘)</label>
                <input type="text" class="w-75 form-control" id="time" name="time" value="<?=$car["time"]?>">
                <input type="hidden" class="w-75 form-control" id="id" name="id" value="<?=$car["id"]?>">
            </div>
            <input type="button" class="btn btn-success w-100 my-3" onclick="aaa()" value="送出" id="">
            <input type="button" class="btn btn-secondary w-100 my-3" onclick="location='./user.php'" value="回上頁" id="">
        </div>
    </form>
</body>
<script>
