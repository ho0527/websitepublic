    ...
    <body>
        ...
        <div class="maincard" id="main">
            <div class="calendar" id="calendar">
                ...
                ?>
                <div class="year">
                    <input type="button" class="button" id="prevmonth" onclick="changeMonth('?year=<?= ($month-1<1)?($year-1):($year) ?>&month=<?= ($month-1<1)?(12):($month-1) ?>')" value="上一個月">
                    <div class="calendartitle"><?= $year ?>年<?= padstart($month,2) ?>月</div>
                    <input type="button" class="button" id="nextmonth" onclick="changeMonth('?year=<?= ($month+1>12)?($year+1):($year) ?>&month=<?= ($month+1>12)?(1):($month+1) ?>')" value="下一個月">
                </div>
                    ...
                    <?php
                        ...
                        for($i=0;$i<42;$i=$i+1){
                            ...
                            if($i<$firstdaymonth||$totalday<$date){
                                ...
                                $time = (intval(date("U",strtotime($day." 00:00:00")))+(8*3600))/(24*3600);
                                ...
                                ?>
                                <div class="calendartd calendardate" id="date_<?= $time ?>" data-day="<?= $day ?>" data-cost="<?= $cost ?>" data-date="<?= $time ?>" data-leftroom="<?= implode(",",$dataleftroom) ?>">
                                    <?= $date ?><br>
                                    $5000<br>
            ...
            <div class="bookroomform">
                ...
                <div class="textcenter">
                    ...
                    <input type="button" class="btn button" onclick="reset()" value="取消">
