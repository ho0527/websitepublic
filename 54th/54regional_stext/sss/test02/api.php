<?php
    ...
    if(isset($_GET["adminpincomment"])){
        ...
        if($row["pin"]=="pin"){
            ...
            ?><script>alert("解除置頂成功");location.href="aaa.php"</script><?php
        }else{
            $row=query("UPDATE `comment` SET `pin`='pin' WHERE `id`=?",[$_GET["adminpincomment"]]);
            ?><script>alert("置頂成功");location.href="aaa.php"</script><?php
        }
    }
