<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>快樂旅遊網</title>
        <link rel="stylesheet" href="bootstrap.css">
        <link rel="stylesheet" href="index.css">
    </head>
    <body>
        <?php include("link.php"); ?>
        <div class="nav" id="nav">
            <div class="title">訪客訂房 - <span id="title2">選擇訂房資訊</span></div>
        </div>

        <div class="maindiv displaynone" id="main2"></div>

        <div class="maincard" id="main">
            <div class="bookroomleft">
                <?php
                    if(isset($_GET["year"])){
                        $year="1970"<=$_GET["year"]?$_GET["year"]:"1970";
                        $month=1<=$_GET["month"]?12>=$_GET["month"]?$_GET["month"]:12:1;
                    }else{
                        $year=date("Y");
                        $month=date("m");
                    }
                ?>
                <div class="flex">
                    <input type="button" class="btn btn-outline-dark" onclick="localStorage.setItem('book',JSON.stringify(book));location.href='?year=<?= $month-1<1?$year-1:$year ?>&month=<?= $month-1<1?12:$month-1 ?>'" value="前一個月">
                    <div><?= padstart(4,$year) ?>年<?= padstart(2,$month) ?>月</div>
                    <input type="button" class="btn btn-outline-dark" onclick="localStorage.setItem('book',JSON.stringify(book));location.href='?year=<?= $month+1>12?$year+1:$year ?>&month=<?= $month+1>12?1:$month+1 ?>'" value="下一個月">
                </div>
                <div class="c">
                    <div>日</div>
                    <div>一</div>
                    <div>二</div>
                    <div>三</div>
                    <div>四</div>
                    <div>五</div>
                    <div>六</div>
                    <?php
                        $firstday=date("w",strtotime("$year-$month-1"));
                        $totaldate=date("t",strtotime("$year-$month-1"));
                        $date=1;
                        for($i=0;$i<42;$i=$i+1){
                            if($i<$firstday||$totaldate<$date){
                                ?><div class="bookroomleftdiv"></div><?php
                            }else{
                                $day=padstart(4,$year)."/".padstart(2,$month)."/".padstart(2,$date);
                                $row=query("SELECT*FROM `bookroom` WHERE (`startday`<=?AND`endday`>=?)",[$day,$day]);
                                $leftroom=[1,2,3,4,5,6,7,8];
                                for($j=0;$j<count($row);$j=$j+1){
                                    $room=explode(",",$row[$j]["room"]);
                                    for($k=0;$k<count($room);$k=$k+1){
                                        unset($leftroom[$room[$k]-1]);
                                    }
                                }
                                array_values($leftroom);
                                ?><div class="bookroomleftdiv bookroomleftdate" id="<?= $date ?>" data-day="<?= $day ?>" data-cost="5000">
                                    <?= $date ?><br>
                                    $5000<br>
                                    <?= count($leftroom) ?>間
                                </div><?php
                                $date=$date+1;
                            }
                        }
                    ?>
                </div>
            </div>
            <div class="bookroomright">
                訂房間數: 1<br><br>
                <div class="margin-5px0px">
                    入住天數
                    <div id="totaldate">請選擇日期</div>
                </div><br>
                <div class="margin-5px0px">
                    入住日期
                    <div id="startday">請選擇日期</div>
                </div>
                <div class="margin-5px0px">
                    ~
                    <div id="endday">請選擇日期</div>
                </div><br>
                <div class="margin-5px0px">
                    房號
                    <div id="room">請選擇日期</div>
                </div><br>
                <div class="text-center">
                    <input type="button" class="btn btn-light" onclick="auto()" value="自動產生房號">
                    <input type="button" class="btn btn-light" value="選擇房號">
                    <input type="button" class="btn btn-light" onclick="localStorage.removeItem('book');location.reload()" value="取消">
                    <input type="button" class="btn btn-warning" id="submit" value="確定訂房">
                </div>
            </div>
        </div>

        <script src="init.js"></script>
        <script src="bookroom.js"></script>
    </body>
</html>